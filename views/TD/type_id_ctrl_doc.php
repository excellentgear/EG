...
<div class="ic-mask" id="helpUseMask"><div class="ic-modal xwide">
    ...
    <div class="m-body help-doc">
        ...
        <h4>其他行為／常見疑問</h4>
        <ul>
            <li>「版別／文件編號」欄：連結自外來文件清單（或其他自動來源）的列無法手動改文字——顯示內容一律即時查詢來源目前狀態（不落地快照，來源異動這裡會跟著變），若來源被刪除會顯示「來源已消失」；手動新增的列可直接輸入文字，也可按「選外來文件」改連結既有附件。欄位旁 <i class="fa fa-eye"></i> 圖示可直接點開附件內容以利確認（本機瀏覽器可預覽的檔案如PDF/圖片會直接開啟預覽，SOP／SIP 會開啟該文件所在的分頁並自動帶入搜尋）；沒有真正版次、退回顯示檔名充當辨識用途時，畫面仍會顯示檔名，但<b>列印不會印出檔名</b>（檔名不是真正的文件編號）。</li>
            <li><b>「選外來文件」按鈕</b>除了外來文件清單附件，也會列出此料號自動符合的 SOP／SIP，以及<b>所有完全不限客戶的「通用」SOP／SIP</b>——後者就是沒有綁定此料號（或客戶）的 SOP／SIP 時，指定通用版供本模組列入的入口。</li>
