...
$ISSUES = [
    ...
    ['高','資材課(倉管組)','3-SM-01 包裝指導書（被裁切的隱藏段落）','檔案裡有兩份「6.相關文件／7.使用表單」，其中一份「7.1 出貨標籤(2-WH-01-07)」被 Word 的固定列高裁掉：該表格列 trHeight=13187 twips(約23.26cm)、hRule=exact、cantSplit，內含 5.5~5.8 包裝照片＋文字共 1025 字，超出部分直接不顯示 → 螢幕看不到、列印也印不出來，但文字仍在檔內（導覽窗格搜尋找得到）。且該編號本身也是錯的：2-WH-01-07 是入庫單，出貨標籤應為 2-WH-01-03（DB／實體檔名／2-WH-01 §9.11 三方一致）。文件最後正常顯示的那份「出貨單(2-WH-01-04)、製程管制卡(2-QA-01-02)」才是正確版。風險：系統線上預覽走 LibreOffice 轉 PDF，不一定照 Word 方式裁切，可能把這段印出來 → Word 看不到、系統預覽看得到，稽核對不上','刪除被裁掉的那份舊 6./7. 段落（不是只改編號），並把該列高度由「固定值」改成「最小值」，避免日後再有內容被無聲裁掉；全檔檢查是否還有其他 hRule=exact 的列'],
