...
if ($verify) {
    ...
    exit((count($bad) + count($dup) + count($ord)) ? 1 : 0);
}
...
$findAppr = $db->prepare("SELECT id, submitted_at, decided_at FROM approval_record
                          WHERE module='form_signer' AND entity_id=? AND level=? AND approver_id=?
                            AND note LIKE '%自動簽核%' AND DATE(decided_at)=? ORDER BY id LIMIT 1");
...
try {
    foreach ($caseIds as $cid) {
        ...
        foreach ($rows as $r) {
            ...
            $new  = eg_auto_sign_next_ts($db, $date, $prev);
            $lastTs = $new;
            if ($new === (string)$r['responded_at']) continue;
            $changed = true; $totResp++;
            if (count($preview) < 12) $preview[] = "case {$r['case_id']} 第{$r['stage_seq']}關 {$r['resolved_user_name']}：{$r['responded_at']} → $new";
            if ($run) {
                $updResp->execute([$new, (int)$r['id']]);
                $findAppr->execute([(int)$r['case_id'], 'stage_' . (int)$r['stage_seq'], (int)$r['resolved_user_id'], $date]);
                if ($a = $findAppr->fetch(PDO::FETCH_ASSOC)) {
                    $updAppr->execute([eg_auto_sign_before_ts($new), $new, (int)$a['id']]);
                    $totAppr++;
                }
            } else {
                $findAppr->execute([(int)$r['case_id'], 'stage_' . (int)$r['stage_seq'], (int)$r['resolved_user_id'], $date]);
                if ($findAppr->fetch(PDO::FETCH_ASSOC)) $totAppr++;
            }
        }
