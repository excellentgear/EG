...
function submitEditBizDate(){
    ...
    if (d === String(c.business_date || '').substring(0, 10)){ closeMask('bizDateMask'); return; }
    var st = ((BIZ_CTX === 'fp') ? FP_BIZ_STATS : CUR_BIZ_STATS) || {auto:0, manual:0};
    if (c.status !== 'draft' && !confirm('確定把業務日期由 ' + dispDate(c.business_date) + ' 改成 ' + dispDate(d) + ' 嗎？\n\n'
        + '文件上 ' + st.auto + ' 個自動簽核的章會改成新日期'
