...
<?php else: ?>
        ...
        <div id="fieldPanel">
            <div class="fsd-toolbar">
                ...
                <button id="btnFpBizDate" style="display:none;" onclick="openEditBizDate('fp')"
                        title="更改這張表單的業務日期（圖章上印的日期、職務回推與 AS 編號版次都以它為準）"><i class="fa fa-calendar"></i> 業務日期</button>
        ...
        <div id="detailPanel">
            <div class="fsd-toolbar">
                ...
                <span id="dtlBizDateInfo" style="margin-left:8px;color:#8a6d45;font-size:12.5px;"></span>
                <button id="btnEditFiller" style="display:none;" onclick="openEditFiller('dtl')"><i class="fa fa-user-edit"></i> 設定填表人</button>
                <button id="btnEditBizDate" style="display:none;" onclick="openEditBizDate('dtl')"
                        title="更改這張表單的業務日期。自動簽核的章會跟著改成新日期，職務與 AS 編號版次也會依新日期回推"><i class="fa fa-calendar"></i> 業務日期</button>
...
<div class="fsd-mask" id="bfCreateMask"><div class="fsd-modal">
    ...
    <div class="m-body">
        ...
        <label style="font-weight:normal;font-size:12px;color:#5b3a1e;margin-top:4px;"
               title="打勾＝列印時右下角不印這個編號。綁定本身照樣成立：AS 文件管理的「填寫紀錄」仍然會列出這一件。用於掃描檔上本來就已經印好編號的紙本。">
            <input type="checkbox" id="bfAsDocHide"> 不在列印頁顯示編號（綁定仍成立，填寫紀錄照樣連動）</label>
        <div id="bfFileWrap">
...
<!-- 業務日期（草稿隨時可改；已送出＝事後編修的一部分，僅超級管理員） -->
<div class="fsd-mask" id="bizDateMask"><div class="fsd-modal">
    <div class="m-head"><span>更改業務日期</span><span class="m-close" onclick="closeMask('bizDateMask')">✕</span></div>
    <div class="m-body">
        <label>業務日期（這張表單「當時」的日期，不是建立案件那天）</label>
        <input type="date" id="bizDateInput" style="width:100%;">
        <div id="bizDateNote" style="margin-top:10px;font-size:12.5px;color:#5b3a1e;line-height:1.7;"></div>
    </div>
    <div class="m-foot"><button class="b-cancel" onclick="closeMask('bizDateMask')">取消</button>
        <button class="b-ok" onclick="submitEditBizDate()">儲存</button></div>
</div></div>
...
<div class="fsd-mask" id="helpUseMask"><div class="fsd-modal" style="max-width:820px;">
    ...
    <div class="m-body help-doc" style="font-size:13px;color:#5b3a1e;line-height:1.8;">
        ...
        <b>①</b>列表右上角按「補案件」，填標題、業務日期（＝所有圖章要印的簽章日期）、可選一份 AS 文件（列印右下角編號，版次依業務日期回推當時生效版），上傳文件掃描檔（多張圖片可拖曳縮圖排頁序，或一份多頁 PDF）。挑了 AS 文件之後下方有一個<b>「不在列印頁顯示編號」</b>勾選框——掃描的紙本上本來就印好編號時勾起來，<b>綁定照樣成立</b>（AS 文件管理的「填寫紀錄」仍然列得到這一件），只是不會再印一次。<br>
...
function bfReloadPeopleForDate(){
    var d = $('#bfDate').val() || '';
    $.getJSON(API, {action:'backfill_meta', date:d}, function(res){
        if (!res.ok) return;
        BF_META.people = res.people || [];
        if (typeof renderBfList === 'function' && BF_FIELDS && BF_FIELDS.length) renderBfList();
    });
}
$(document).on('change', '#bfDate', bfReloadPeopleForDate);
...
function openBfCreate(caseObj){
    bfEnsureMeta(function(){
        ...
        $('#bfAsDocHide').prop('checked', isEdit ? !!Number(caseObj.as_doc_hide_print) : false);
        renderBfAsDocLabel();
...
function submitBfCreate(){
    ...
    if (BF_EDIT_CASE){
        $.post(API, {action:'backfill_update_head', csrf:META.csrf, case_id:BF_EDIT_CASE.id,
                     title:title, business_date:bizDate, as_doc_id:BF_AS_DOC_ID,
                     as_doc_hide_print:$('#bfAsDocHide').prop('checked')?1:0}, function(res){
            if (!res.ok){ alert(res.error||'儲存失敗'); return; }
            closeMask('bfCreateMask');
            if (FP_CASE && String(FP_CASE.id)===String(BF_EDIT_CASE.id)) { FP_CASE.title=res.case.title; FP_CASE.business_date=res.case.business_date; FP_CASE.as_doc_id=res.case.as_doc_id; FP_CASE.as_doc_hide_print=res.case.as_doc_hide_print; $('#fpTitle').text(FP_CASE.title||''); }
            BF_EDIT_CASE = null;
        }, 'json');
        return;
    }
    ...
    fd.append('as_doc_hide_print', $('#bfAsDocHide').prop('checked') ? 1 : 0);
...
function openCase(id){
    $.getJSON(API, {action:'case_get', id:id}, function(res){
        ...
        // 業務日期：詳情頁本來看不到它（只有列表有），但章上印的日期、職務與 AS 版次全以它為準，一定要看得見
        CUR_BIZ_STATS = res.biz_date_stats || {auto:0, manual:0};
        // 章的日期跟業務日期對不起來要主動講出來——不標的話沒人會發現文件上印的是別的日期
        $('#dtlBizDateInfo').html('業務日期：' + dispDate(CUR_CASE.business_date)
            + (CUR_BIZ_STATS.mismatch > 0
                ? ' <span style="color:#DD5138;font-weight:bold;">（文件上有 ' + CUR_BIZ_STATS.mismatch + ' 個章印的是 '
                  + (CUR_BIZ_STATS.stamp_dates || []).map(function(d){ return dispDate(d); }).join('、')
                  + '，按「業務日期」可對齊）</span>'
                : ''));
        $('#btnEditBizDate').toggle(!!res.can_set_biz_date);
...
// 改業務日期前要先知道「幾個章會跟著改、幾個不會」（自動簽核 vs 真人當場簽）
var CUR_BIZ_STATS = null, FP_BIZ_STATS = null;
...
/* ---------- 業務日期（章上印的日期、職務回推、AS 編號版次都以它為準） ----------
 * 一般案件章上的日期印的是**那一筆簽核紀錄的時間**：自動簽核的時間是送出當下依業務日期算的，
 * 所以改業務日期一定要連動重排，否則會出現「業務日期 09.01、章卻印 08.25」這種對不起來的文件。
 * 真人當場按核准的那幾筆是實際發生的事，不會被改（跳窗裡會講明有幾個章日期不變）。 */
var BIZ_CTX = 'dtl';
function openEditBizDate(ctx){
    BIZ_CTX = (ctx === 'fp') ? 'fp' : 'dtl';
    var c = (BIZ_CTX === 'fp') ? FP_CASE : CUR_CASE;
    if (!c) return;
    var st = ((BIZ_CTX === 'fp') ? FP_BIZ_STATS : CUR_BIZ_STATS) || {auto:0, manual:0};
    $('#bizDateInput').val(String(c.business_date || '').substring(0, 10));
    var h = '';
    if (c.status === 'draft') {
        h = '這件還是草稿，改日期只會換掉這個欄位；送出時才會依它決定圖章上的日期、當時的部門職稱與 AS 編號版次。';
    } else {
        // 章的日期跟業務日期對不起來（多半是業務日期事後被改過，章還停在舊日期）：
        // 這時直接按儲存（日期不必改）就會把章對齊回業務日期
        if (st.mismatch > 0)
            h += '<div style="background:#F7E0BD;border:1px solid #E0B87A;border-radius:4px;padding:8px;margin-bottom:8px;color:#5b3a1e;">'
               + '<b>目前文件上有 ' + st.mismatch + ' 個章的日期跟業務日期對不起來</b>：章上印的是 '
               + (st.stamp_dates || []).map(function(d){ return dispDate(d); }).join('、')
               + '，業務日期是 ' + dispDate(c.business_date) + '。<b>直接按「儲存」（日期不必改）就會把這些章對齊成業務日期。</b></div>';
        h += '<b>改完會一起發生的事：</b><br>'
          + '・文件上 <b>' + st.auto + '</b> 個由系統自動簽核的章，日期會跟著改成新的業務日期（先後順序不變）。<br>'
          + (st.manual > 0
              ? '・<b style="color:#a13a24;">有 ' + st.manual + ' 個章是本人當場按下核准的，日期不會變</b>——那是實際簽核的時間，改掉等於假造紀錄。<br>'
              : '')
          + '・圖章上的部門職稱、列印右下角的 AS 編號版次，改成依新日期回推。<br>'
          + '・已存檔的合成 PDF 會作廢，下次開啟案件時用新日期重新產生。<br>'
          + '・會留下稽核紀錄（誰、什麼時候、從哪一天改到哪一天）。';
    }
    $('#bizDateNote').html(h);
    openMask('bizDateMask');
}
function submitEditBizDate(){
    var c = (BIZ_CTX === 'fp') ? FP_CASE : CUR_CASE;
    if (!c) return;
    var d = String($('#bizDateInput').val() || '');
    if (!d){ alert('請選擇業務日期'); return; }
    var st = ((BIZ_CTX === 'fp') ? FP_BIZ_STATS : CUR_BIZ_STATS) || {auto:0, manual:0};
    var same = (d === String(c.business_date || '').substring(0, 10));
    // 日期沒變也可能有事要做：章的日期跟業務日期對不起來時，儲存＝把章對齊回業務日期
    if (same && !(st.mismatch > 0)){ closeMask('bizDateMask'); return; }
    if (c.status !== 'draft' && !confirm(
        (same ? '確定把文件上 ' + st.mismatch + ' 個對不起來的章，日期對齊成業務日期 ' + dispDate(d) + ' 嗎？\n\n'
              : '確定把業務日期由 ' + dispDate(c.business_date) + ' 改成 ' + dispDate(d) + ' 嗎？\n\n')
        + '文件上 ' + st.auto + ' 個自動簽核的章會重新排到 ' + dispDate(d)
        + (st.manual > 0 ? '；' + st.manual + ' 個本人當場簽的章日期不變' : '')
        + '，已產生的合成 PDF 會作廢並在下次開啟時重新產生。')) return;
    $.post(API, {action:'case_set_biz_date', csrf:META.csrf, case_id:c.id, business_date:d}, function(res){
        if (!res.ok){ alert(res.error||'更改失敗'); return; }
        closeMask('bizDateMask');
        if (BIZ_CTX === 'fp'){ openFieldDesigner(c.id); } else { openCase(c.id); }
    }, 'json');
}
...
function fsdBoxContent(f, r){
    ...
    if (f.box_type === 'stamp' && CUR_CASE && (CUR_CASE.case_kind === 'backfill' || f.signer_name)){
        ...
        // 圖章模板有 {部門}{職稱} token 時才會顯示；沒有 token 傳了也不影響。
        if (f.signer_name && window.EGStamp) return {kind:'stamp', html:EGStamp.stamp(f.signer_name, dispDate(CUR_CASE.business_date), false, bfSchema, f.signer_dept||'', f.signer_position||'')};
        return null;
    }
    if (f.box_type === 'stamp'){
        // 圖章日期一律走 dispDate() 顯示成 YYYY.MM.DD(ai-rules/20)，不可直接丟原始的 YYYY-MM-DD
        if (r && r.decision && r.decision !== 'skipped_sod' && window.EGStamp)
            return {kind:'stamp', html:EGStamp.stamp(r.resolved_user_name, dispDate((r.responded_at||'').substring(0,10)), false, fsdCaseStampSchema(), r.sign_dept||'', r.sign_position||'')};
...
function openFieldDesigner(id){
    $.getJSON(API, {action:'case_get', id:id}, function(res){
        ...
        // 業務日期：補案件草稿在「編輯表頭」裡就改得到，這顆是給一般案件草稿與事後編修用的
        FP_BIZ_STATS = res.biz_date_stats || {auto:0, manual:0};
        $('#btnFpBizDate').toggle(!!res.can_set_biz_date && (!FP_BACKFILL || FP_POST_EDIT));
...
/* ?case_id=N 直接開該案件詳情（AS 文件管理的「填寫紀錄」點過來用；PDF 還沒產生時就是靠這條路
   進來讓它自動補產）。清單仍照常載入，按「返回列表」就回得去。 */
loadMeta(function(){
    loadCases();
    var qCase = parseInt(new URLSearchParams(location.search).get('case_id'), 10) || 0;
    if (qCase) openCase(qCase);
});
</script>
</body>
</html>
