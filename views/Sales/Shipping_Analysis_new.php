...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && ($_POST['action'] ?? '') === 'get_kpi_data') {
    ...
    try {
        ...
        $g_cutoff = 0;
        try {
            $g_cutoff = intval($pdo->query("SELECT setting_value FROM system_settings WHERE setting_key='billing_cutoff_day' LIMIT 1")->fetchColumn());
        } catch(Exception $_ge) {}
        ...
        for ($i = 0; $i < 4; $i++) {
            ...
            $result_weeks[] = [
                ...
                'start'         => $weeks[$i]['start'],
                'end'           => $weeks[$i]['end'],
                'week_target'   => round($week_target),
                'cum_target'    => round($cum_target),
                'order_amount'  => round($d['order_amount']),
                'order_rate'    => $week_target > 0 ? round($d['order_amount'] / $week_target * 100, 2) : 0,
                'ship_amount'   => round($d['ship_amount']),
                'return_amount' => round($d['return_amount']),
                'revenue'       => round($revenue),
                'cum_revenue'   => round($cum_revenue),
        ...
        echo json_encode([
            ...
            'weeks'         => $result_weeks,
            'billing_start' => $_bd_start->format('Y-m-d'),
            'billing_end'   => $_bd_end->format('Y-m-d'),
            'totals'        => [
