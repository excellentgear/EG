...
<div class="va-mask" id="helpUseMask"><div class="va-modal wide">
    ...
    <div class="m-body help-doc">
        ...
        <ul>
            ...
            <li><b>快速套用</b>（工具列，臨時關鍵字）：臨時想到一組關鍵字時用這裡，<b>不必取規則名稱、不會存進規則表</b>。填「包含」「不包含」、選好要套用的標籤、按<b>掃描</b>，命中的項目會逐筆列出來（含<b>每一筆目前的製程標籤</b>，料號一樣可以點開圖面），預設全部勾選，可以「全部取消／反向勾選」，也可以<b>按住 Shift 點第二個勾選框連續選取一整段</b>。最後決定是要把標籤<b>增加</b>上去（保留原有的）還是<b>移除</b>掉。「對象」可限定<b>只要還沒設定製程的</b>或<b>只要已經設定過的</b>。<br>
            <b>依「目前的製程標籤」篩選</b>（跟上面「要套用的標籤」是兩回事）：<b>包含</b>＝至少含有選的那幾個、可以還有別的；<b>絕對相同</b>＝標籤組合完全一樣，不能多也不能少（不管順序）。<b>有設標籤篩選時「包含」關鍵字可以留空</b>，例如「把所有『粗滾＋齒研』的項目移除齒研」就只要設絕對相同再選移除即可。關鍵字與標籤篩選同時設＝兩個條件都要成立；兩個都空會被擋下（那等於全部項目，不是篩選）。<br><b>套用之後「包含／不包含」與已選標籤都會自動清空</b>（掃描結果一併清掉），避免接著又按一次套用、用的卻是上一批的條件；要繼續處理剩下的請重新輸入關鍵字再按「掃描」。<br>兩種動作的差別：<b>增加</b>不會刪掉原本的製程對照（舊資料有「有製程卻沒有標籤紀錄」的情況，整組重建會把原本的製程默默清掉）；<b>移除</b>會依剩下的標籤重建，所以只作用在「真的有標籤紀錄」的項目，其餘會略過並在結果告訴您幾筆沒動到。</li>
