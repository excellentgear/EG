...
<head>
    ...
    <style>
        ...
        .pick-hint { background:#DD5138; color:#fff; font-size:12px; padding:4px 8px; border-radius:4px; margin-bottom:6px; }
        /* 批次標籤面板：貼在畫面右緣、蓋在跳窗之上但在標籤挑選跳窗之下 */
        #kwSidePanel { position:fixed; top:0; right:0; bottom:0; width:330px; background:#fff; z-index:1070;
            box-shadow:-4px 0 18px rgba(0,0,0,.25); display:none; flex-direction:column; }
        #kwSidePanel.on { display:flex; }
...
<div class="va-mask" id="helpUseMask"><div class="va-modal wide">
    ...
    <div class="m-body help-doc">
        ...
        <ul>
            ...
            <li><b>快速套用</b>（工具列，臨時關鍵字）：臨時想到一組關鍵字時用這裡，<b>不必取規則名稱、不會存進規則表</b>。填「包含」「不包含」、選好要套用的標籤、按<b>掃描</b>，命中的項目會逐筆列出來（含<b>每一筆目前的製程標籤</b>，料號一樣可以點開圖面），預設全部勾選，可以「全部取消／反向勾選」，也可以<b>按住 Shift 點第二個勾選框連續選取一整段</b>。最後決定動作：<b>增加</b>（保留原有的）、<b>移除</b>（只拿掉這幾個、其餘保留）、<b>取代</b>（整組換成您選的，<b>原有的標籤會被清掉</b>）。「對象」可限定<b>只要還沒設定製程的</b>或<b>只要已經設定過的</b>。<br>
            <b>依「目前的製程標籤」篩選</b>（跟上面「要套用的標籤」是兩回事）：<b>包含</b>＝至少含有選的那幾個、可以還有別的；<b>絕對相同</b>＝標籤組合完全一樣，不能多也不能少（不管順序）。<b>有設標籤篩選時「包含」關鍵字可以留空</b>，例如「把所有『粗滾＋齒研』的項目移除齒研」就只要設絕對相同再選移除即可。關鍵字與標籤篩選同時設＝兩個條件都要成立；兩個都空會被擋下（那等於全部項目，不是篩選）。<br><b>套用之後「包含／不包含」與已選標籤都會自動清空</b>（掃描結果一併清掉），避免接著又按一次套用、用的卻是上一批的條件；要繼續處理剩下的請重新輸入關鍵字再按「掃描」。<br>三種動作的差別：<b>取代</b>＝與偵測跳窗按「套用」同一個行為（整組換掉）；<b>增加</b>不會刪掉原本的製程對照（舊資料有「有製程卻沒有標籤紀錄」的情況，整組重建會把原本的製程默默清掉）；<b>移除</b>會依剩下的標籤重建，所以只作用在「真的有標籤紀錄」的項目，其餘會略過並在結果告訴您幾筆沒動到。</li>
            ...
            <li><b>批次標籤面板</b>（偵測跳窗工具列）：開在畫面右緣，<b>分成上下兩組</b>，可以一次擺好兩批不同的標籤再各自套用（現場常是「這幾筆齒研、那幾筆滾齒」）。把左邊每一列最右邊的 <b>&#9776;</b> 拖進任一組即可，<b>拖進來的項目會從左邊清單移走</b>（避免同一筆重複處理），按項目後面的 <b>×</b> 可原樣放回原本那一組。每一組各自選標籤（可連續多選）、各自有「改用增加」（保留原有標籤）與「套用這一組」。拖曳時面板會自動打開。<br><b>套用之後那一組的項目與已選標籤都會自動清空</b>，避免下一批拖進來時沿用上一批的標籤而套錯（「改用增加」的勾選會保留）。</li>
...
<div class="va-mask" id="quickApplyMask"><div class="va-modal wide">
    ...
    <div class="m-body">
        ...
        <div style="margin-top:8px;font-size:12px;display:flex;gap:14px;flex-wrap:wrap;align-items:center;
                    background:#FBF6EC;border:1px solid #EADFC8;border-radius:4px;padding:5px 8px;">
            <span><b>依「目前的製程標籤」篩選</b> <span style="color:#888;">（可不設；有設的話「包含」關鍵字可以留空）</span></span>
            <button class="btn btn-default btn-xs" onclick="qaPickFilterTags()"><i class="fa fa-filter"></i> 選擇標籤</button>
            <span id="qaFTagText" style="color:#8a5a2b;">未設定</span>
            <label><input type="radio" name="qaFMode" value="contains" checked> <b>包含</b>這些標籤（可以還有別的）</label>
            <label><input type="radio" name="qaFMode" value="exact"> <b>絕對相同</b>（組合完全一樣，不管順序）</label>
            <a href="javascript:void(0)" onclick="qaClearFilterTags()" style="font-size:11px;">清除</a>
        </div>
        <div style="margin-top:8px;font-size:12px;display:flex;gap:16px;flex-wrap:wrap;">
            <span><b>動作</b>
                ...
                <label style="margin-left:8px;"><input type="radio" name="qaMode" value="remove"> <b>移除</b>這些標籤</label>
                <label style="margin-left:8px;"><input type="radio" name="qaMode" value="replace"> <b>取代</b>：設為這些標籤（<span style="color:#DD5138;">原有的會被清掉</span>）</label></span>
            <span><b>範圍</b>
        ...
        <div style="max-height:380px;overflow:auto;border:1px solid #EADFC8;border-radius:4px;">
            <table class="kw-item-table"><thead><tr style="background:#FBF6EC;position:sticky;top:0;z-index:1;box-shadow:0 1px 0 #E4C293;">
                <th style="width:120px;padding:4px 6px;">報價單</th><th style="width:110px;">客戶</th>
                <th style="width:150px;">料號</th><th>規格</th><th style="width:170px;">目前的製程標籤</th>
                <th style="width:34px;text-align:center;">選取</th>
...
function runKwScan() {
    ...
    $.post(API_URL, { action: 'qkw_scan', quote_ids: JSON.stringify(scopeAll ? [] : rows.map(function(r){ return Number(r.quote_id); })),
        ...
        kwChecked = {};
        // 預設一個都不勾（使用者要求）：套用會直接寫進資料，全部先勾起來很容易一按就整批套用出去。
        // 要整批確認請按「全部勾選」，或用「套用到這裡為止」逐段推進。
...
function kwToggleOpen(gi) {
    if (kwPickMode) { kwApplyUpto(gi); return; }   // 指定模式下點分組列＝以這一組為界，不是展開
    const $g = $('#kwG' + gi);
...
function submitKwApply(okFn) {
    const batches = [];
    const appliedSet = {};
    let total = 0;
    kwGroups.forEach(function(g, gi) {
        // okFn＝「套用到這裡為止」用的界線判斷；沒給就是全部已勾選的
        const ids = g.items.filter(function(it){ return kwChecked[it.item_id] && (!okFn || okFn(it, gi)); })
                           .map(function(it){ return it.item_id; });
        if (!ids.length) return;
        ids.forEach(function(id){ appliedSet[id] = true; });
        total += ids.length;
    ...
    $.post(API_URL, { action: 'qkw_apply', batches: JSON.stringify(batches) }, function(res) {
        ...
        kwGroups.forEach(function(g) {
            g.items.forEach(function(it){ if (appliedSet[it.item_id]) applied.push(it); });
        });
        kwApplyLocalUpdate(applied);
        kwGroups.forEach(function(g) {
            g.items = g.items.filter(function(it){ return !appliedSet[it.item_id]; });
...
function tagDefaultGid(sel) {
    if (!processTagTree.length) return null;
    let gid = processTagTree[0].group_id;
    if (sel && sel.length) processTagTree.forEach(function(g) {
        if ((g.sub_tags || []).some(function(st){ return st.sub_tag_id === sel[0]; })) gid = g.group_id;
    });
    return gid;
}

function tagPickRender() {
    if (tagPickGid === null) tagPickGid = tagDefaultGid(tagPickSel);
    $('#tagPickArea').html(tagPickerHtml(tagPickSel, tagPickGid, 'tagPick'));
}
...
function kwSlotApply(si) {
    ...
    const done = function(res, msg) {
        ...
        // 套用完連「已選的標籤」一起清掉（使用者要求）：標籤留著的話，
        // 下一批拖進來就會沿用上一批的標籤，很容易忘了改而套錯
        sl.items = []; sl.tags = [];
        kwSlotRender(si);
...
// 逐組確認完就套用到那裡，前面的先落地、後面的原封不動。
let kwPickMode = false;
function kwTogglePick() {
    kwPickMode = !kwPickMode;
    $('#kwGroups').toggleClass('pick-mode', kwPickMode);
    $('#kwPickHint').toggle(kwPickMode);
    $('#btnKwUpto').toggleClass('btn-warning', kwPickMode).toggleClass('btn-default', !kwPickMode);
}
// 界線以「項目」計算（不是以組），這樣點分組列或點組內的某一列都能當界線
function kwFlatOrder() {
    const map = {}; let k = 0;
    kwGroups.forEach(function(g){ g.items.forEach(function(it){ map[it.item_id] = k++; }); });
    return map;
}
function kwApplyUptoFlat(boundary, label) {
    const flat = kwFlatOrder();
    let n = 0;
    kwGroups.forEach(function(g){ g.items.forEach(function(it){ if (kwChecked[it.item_id] && flat[it.item_id] <= boundary) n++; }); });
    if (!n) { qtNotify('這個位置以上沒有勾選任何項目。', 'warn'); return; }
    if (!confirm('將只套用到「' + label + '」為止、已勾選的 ' + n + ' 筆項目。這個位置以後的完全不動，確定嗎？')) return;
    kwTogglePick();
    submitKwApply(function(it){ return flat[it.item_id] <= boundary; });
}
function kwApplyUpto(gi) {          // 點分組列＝到那一組的最後一筆為止
    const g = kwGroups[gi];
    if (!g || !g.items.length) { qtNotify('這一組沒有項目。', 'warn'); return; }
    kwApplyUptoFlat(kwFlatOrder()[g.items[g.items.length - 1].item_id], '第 ' + (gi + 1) + ' 組的最後一筆');
}
// 展開後點組內的某一列＝到那一筆為止（勾選框與料號連結照常運作）
$(document).on('click', '.pick-mode table.kw-item-table tr', function(e) {
    if ($(e.target).is('input,a') || $(e.target).closest('a').length) return;
    const iid = Number($(this).find('.kw-item-chk').val());
    if (!iid) return;
    const gi = Number($(this).find('.kw-item-chk').data('gi'));
    kwApplyUptoFlat(kwFlatOrder()[iid], '第 ' + (gi + 1) + ' 組裡的這一筆');
});

// ── 快速套用（臨時關鍵字，不存規則）────────────────────────
let qaTags = [], qaItems = [], qaChecked = {}, qaLastIdx = -1;
let qaFTags = [];      // 依「項目目前的製程標籤」篩選（與要套用的標籤是兩回事，不要混用）

function qaPickFilterTags() {
    tagPickOpen('選擇要篩選的製程標籤',
        '這是用來<b>找出目前已經掛著這些標籤的項目</b>，跟上面「要套用的標籤」是兩回事。<br>' +
        '<b>包含</b>＝至少含有這些（可以還有別的）；<b>絕對相同</b>＝標籤組合完全一樣，不能多也不能少。', qaFTags, function(ids) {
            qaFTags = ids;
            $('#qaFTagText').text(ids.length ? tagNamesOf(ids) : '未設定');
        });
}
function qaClearFilterTags() { qaFTags = []; $('#qaFTagText').text('未設定'); }
...
function qaScan() {
    ...
    // 有設標籤篩選時，關鍵字可以留空（例：把所有「粗滾＋齒研」的改成只有粗滾）
    const needKw = !inc && !qaFTags.length;
    $('#qaIncErr').text(needKw ? '請填「包含」關鍵字，或改用下方的標籤篩選' : '');
    $('#qaTagErr').text(qaTags.length ? '' : '請先選擇要套用的製程標籤');
    if (needKw || !qaTags.length) return;
    ...
    $.post(API_URL, { action: 'qkw_quick_scan', include_kw: inc,
                      ...
                      filter: $('input[name="qaFilter"]:checked').val(),
                      filter_tags: JSON.stringify(qaFTags),
                      filter_mode: $('input[name="qaFMode"]:checked').val() }, function(res) {
        ...
        $('#qaSummary').html('命中 <b>' + res.data.total + '</b> 筆' +
            ...
            '　範圍：' + (all ? '全部年份' : '目前篩選') +
            (qaFTags.length ? ('　標籤篩選：' + ($('input[name="qaFMode"]:checked').val() === 'exact' ? '絕對相同' : '包含') +
                               ' ' + escapeQt(tagNamesOf(qaFTags))) : ''));
        qaRender();
    });
}
...
function qaRender() {
    ...
    qaItems.forEach(function(it, i) {
        // 勾選欄放最右邊：使用者是對著右邊的「目前製程標籤」在判斷要不要選，勾選框在左邊要一直來回看
        h += '<tr>' +
            ...
            '<td style="width:34px;text-align:center;"><input type="checkbox" class="qa-chk" data-i="' + i + '" value="' + it.item_id + '"' + (qaChecked[it.item_id] ? ' checked' : '') + '></td>' +
        '</tr>';
    });
    $('#qaRows').html(h);
    qaSync();
...
// 指定模式：點某一列＝只套用那一列以上（含）而且有勾選的
let qaPickMode = false;
function qaTogglePick() {
    qaPickMode = !qaPickMode;
    $('#qaRows').toggleClass('pick-mode', qaPickMode);
    $('#qaPickHint').toggle(qaPickMode);
    $('#btnQaUpto').toggleClass('btn-warning', qaPickMode).toggleClass('btn-default', !qaPickMode);
}
$(document).on('click', '#qaRows.pick-mode tr', function(e) {
    if ($(e.target).is('input,a') || $(e.target).closest('a').length) return;   // 勾選框與料號連結照常
    const i = Number($(this).find('.qa-chk').data('i'));
    if (isNaN(i)) return;
    const n = qaItems.slice(0, i + 1).filter(function(it){ return qaChecked[it.item_id]; }).length;
    if (!n) { qtNotify('這一列以上沒有勾選任何項目。', 'warn'); return; }
    if (!confirm('將只套用「第 1 列 ～ 第 ' + (i + 1) + ' 列」之間已勾選的 ' + n + ' 筆。第 ' + (i + 2) + ' 列以後完全不動，確定嗎？')) return;
    qaTogglePick();
    qaApply(i);
});

function qaApply(maxIdx) {
    const upto = (maxIdx === undefined || maxIdx === null) ? qaItems.length - 1 : Number(maxIdx);
    const ids = qaItems.slice(0, upto + 1).filter(function(it){ return qaChecked[it.item_id]; }).map(function(it){ return it.item_id; });
    ...
    const verb = mode === 'add' ? '增加' : (mode === 'remove' ? '移除' : '取代');
    const note = mode === 'add' ? '（原有的標籤會保留）'
               : (mode === 'remove' ? '（只會拿掉這些標籤，其餘保留）'
                                    : '（這些項目原本的標籤會被清掉，改成上面選的）');
    if (!confirm('將把已勾選的 ' + ids.length + ' 筆項目的製程標籤' + verb + '為「' + tagNamesOf(qaTags) + '」。' + note + ' 確定要執行嗎？')) return;
    const $btn = $('#qaApplyBtn').prop('disabled', true).html('<i class="fa fa-spinner fa-spin"></i> 套用中…');
    // 「取代」就是既有 qkw_apply 的行為（整組換掉），直接沿用同一支，不在 adjust_tags 裡再刻一份
    const payload = (mode === 'replace')
        ? { action: 'qkw_apply', batches: JSON.stringify([{ sub_tag_ids: qaTags.join(','), item_ids: ids }]) }
        : { action: 'qkw_adjust_tags', item_ids: JSON.stringify(ids), sub_tag_ids: JSON.stringify(qaTags), mode: mode };
    $.post(API_URL, payload, function(res) {
        ...
        // 套用完把條件與標籤一起清掉（使用者要求）：留著的話很容易接著又按一次套用，
        // 用的卻是上一批的關鍵字與標籤。清完不重掃（沒有條件可掃），請重新輸入再按掃描。
        qaReset('已' + verb + '標籤 ' + res.data.items + ' 筆。條件與標籤已清空，請重新輸入關鍵字再按「掃描」。');
        const keepPage = qtPage;
        loadPendingList(function(){ qtPage = keepPage; renderCards(); });
    });
}

// 清空快速套用的條件、標籤與掃描結果（回到剛打開跳窗的狀態）
function qaReset(msg) {
    if (typeof qaPickMode !== 'undefined' && qaPickMode) qaTogglePick();
    $('#qaInc').val(''); $('#qaExc').val('');
    $('#qaIncErr').text(''); $('#qaTagErr').text('');
    qaTags = []; $('#qaTagText').text('尚未選擇');
    qaFTags = []; $('#qaFTagText').text('未設定');
    qaItems = []; qaChecked = {}; qaLastIdx = -1;
    $('#qaRows').html('');
    $('#qaSummary').html(msg ? ('<span style="color:#8a5a2b;">' + escapeQt(msg) + '</span>') : '');
    qaSync();
}
