    ...
    <div class="modal-box" style="min-width:480px;">
        ...
        <div class="modal-body">
            ...
            <div id="pf-save-hint" style="font-size:11.5px;color:#8b949e;margin-bottom:10px;">
                會存兩個附件：<b>壓平 PNG</b>＋<b>工作檔 .egwork.json</b>（用下方「開啟」重新載入後，標籤/文字/球標全部仍可編輯）。<b>這條路徑存出來的東西＝暫存，只有在這個編輯器裡看得到</b>——連壓平 PNG 也<b>不會出現在料號附件、圖面查閱、外來文件清單</b>等任何其他頁面（不分登入者，避免被當成正式圖面造成誤會）。也因此不填發行章日期、不做圖面變更判定；<b>要正式出圖請勾「只存圖片，不建立工作檔」</b>。分享範圍只管「誰能開這份工作檔繼續編輯」，沒有「全公司共用」，避免所有人都能改到；同一料號最多保留 <?= (int)$workfileMaxCount ?> 份，超過會自動刪掉最舊的一份（不影響剛存好的這份，也不會刪到圖）。
            </div>
            <hr style="border-color:#3c4046;margin:10px 0;">
...
<?php if ($show_gear_tool): ?>
                <li><b>齒輪計算</b>（底部狀態列）：開啟<b>齒輪／花鍵計算工具</b>浮動視窗，可以一邊看圖一邊算（視窗可拖曳移動，關掉不影響畫布內容）。與<b>訂單追蹤頁的「齒輪計算」是同一個工具、同一份預留量與花鍵公差資料</b>，兩邊算出來一定一致。分頁包含：基本齒輪計算、跨齒⇄跨珠互換、跨珠換算、建議滾齒值、由外徑或跨齒厚回推轉位係數 x、內／外花鍵與花鍵配合、矩形栓槽跨銷值、出尾長度與刀具外徑建議，以及模數／外徑預留量的管理。沒有出現這顆按鈕＝你的角色沒有「齒輪計算」權限，請洽管理者到<b>使用者權限管理 →「未交訂單／訂單追蹤」</b>區塊勾選</li>
<?php endif; ?>
