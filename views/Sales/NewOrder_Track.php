...
<head>
    ...
    <style>
        ...
        /* ── 鍵槽計算工具：CSS 已抽成共用檔 views/Sales/_keyway_tool_ui.php（2026-09-03）──
           該檔在頁尾 include，CSS／視窗 HTML／JS 都在裡面，本頁不再各留一份（鐵律4）。 */
    </style>
</head>
<body class="nav-sm">
...
<?php
// ── 鍵槽計算工具 ───────────────────────────────────────────────────────────
// 2026-09-03：原本 CSS＋視窗 HTML＋JS 都內嵌在本頁，已抽成共用檔，
// 讓批圖編輯器（image_editor.php）等頁面 include 同一份（禁止再複製一套＝鐵律4）。
//   畫面：views/Sales/_keyway_tool_ui.php｜權限：src/common/keyway_tool_lib.php
// 顯示與否沿用本頁上方已算好的 $can_keyway_calc。
...
$PAGE_HELP_TITLE = '訂單追蹤 使用說明';
$PAGE_HELP_BODY  = <<<'HTMLHELP'
<p>本頁是<b>業務端的訂單總表</b>：從接單、指定設計、批圖溝通、轉生管，一路追到完結。上方的統計卡片同時也是<b>篩選鈕</b>，點一下就只看那一類。</p>
...
<h4>二、搜尋</h4>
<ul>
    <li>搜尋框是<b>全表搜尋</b>：客戶、料號、製程、業務備註、設計備註、設計者、各種單號、日期都掃得到。</li>
    <li>打<b>多個關鍵字</b>（空白分隔）＝每個都要命中，可以分散在不同欄位。</li>
</ul>

<h4>三、設計備註與溝通紀錄</h4>
<ul>
    <li>設計備註直接在欄位裡打字，按 Enter 存檔。</li>
    <li>打了字之後會出現【<b>已處理</b>】：按下去會把<b>畫面上當下的內容</b>（含還沒按 Enter 的新字）存成一筆<b>溝通紀錄</b>並清空欄位，之後可以繼續打新的。</li>
    <li>【<b>溝通紀錄 N</b>】唯讀，看得到這一頁的人都能開：依時間新→舊列出處理人與內容，還沒處理的當前內容會置頂另外標示。</li>
    <li>如果同一列別人剛好也改過，按下已處理時會擋下並把畫面換成最新內容請您再確認一次——<b>不會覆蓋、也不會安靜吃掉別人的字</b>。</li>
</ul>

<h4>四、客戶欄的鎖頭</h4>
<ul>
    <li>訂單一旦綁定料號，<b>客戶就由料號決定</b>（存檔時後端會用料號的客戶覆寫），所以客戶欄是鎖住的。</li>
...
<h4>五、急件標示</h4>
<ul>
    <li>急件徽章顯示在客戶欄：<b>一般件</b>／<b>急件 U</b>／<b>特急件 E</b>（E 最急），全站同一組顏色。</li>
</ul>
...
<h4>七、其他常用動作</h4>
<ul>
    <li><b>轉生管</b>／取消轉生管、<b>結案</b>／解除完結、<b>暫停／取消訂單</b>、<b>OP 轉訂單</b>、<b>訂單變更</b>：都依角色權限顯示。</li>
    <li><b>批圖編輯器</b>、<b>前往料號主檔編輯</b>：需要對應權限。</li>
    <li><b>標籤</b>：側欄可搜尋；標籤管理跳窗也有搜尋（名稱／#標示／分類），並會顯示「顯示 N / 共 M 個」。
        <b>改關鍵字時被篩掉的選取會自動取消</b>，避免「畫面寫已選 12 個、只看得到 3 個」卻把看不見的一起刪掉。</li>
</ul>

<h4>八、權限角色</h4>
<ul>
    <li>頁首標題旁會顯示您目前的權限代碼與角色。功能分成<b>訂單流程／訂單變更／設計與批圖／計算工具</b>四群，逐項由角色勾選。</li>
    <li>角色與指派在<b>系統設定 → 使用者權限設定</b>；前端擋掉的動作，後端一律再擋一次。</li>
</ul>
HTMLHELP;
