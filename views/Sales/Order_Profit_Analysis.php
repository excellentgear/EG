...
function opa_auto_match(PDO $pdo, array $orders, array $bomsByOrder): array {
    ...
    foreach ($ordByDid as $did => $ords) {
        ...
        foreach ($ords as $o) {
            $need = intval($o['Qty']);
            $odate = substr(strval($o['Order_date'] ?? ''), 0, 10);
            $alloc = [];
            while ($need > 0 && $pi < count($pool)) {
                ...
                // 日期護欄：製令建立日早於訂單日 → 不自動建議（早於訂單的多為出庫存貨，只該手動綁）。
                // 池已由新到舊排序，一旦遇到更舊的製令，之後只會更舊，故對本單停止分配；
                // 該製令仍保留給後續（更舊）的訂單比對。
                $bdate = substr(strval($pool[$pi]['Created_At'] ?? ''), 0, 10);
                if ($bdate !== '' && $odate !== '' && $bdate < $odate) break;
...
<?php else: ?>
      ...
      <div class="main-card">
        ...
        <div class="note-box">
          ...
          ．未綁定製令的訂單會<b>自動比對批次</b>（標「≈」）：同料號之下，訂單由新到舊、未綁定製令由新到舊，依數量逐批往前分配；僅供計算顯示，<b>不寫入任何綁定</b>。<b>製令建立日早於訂單日者不列入自動建議</b>（早於訂單多為出庫存貨，只該手動綁）。<br>
