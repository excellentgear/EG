...
<script>
    ...
    $(document).ready(function() {
        ...
        $('#pdf-btn').on('click', function() {
            // 使用 html2canvas 擷取 x_panel 區塊
            const { jsPDF } = window.jspdf;
            const elementToCapture = document.querySelector('.x_panel');
            exportElementAsPDF(elementToCapture, '超正齒輪-組織圖.pdf', '.panel_toolbox, .navbar-right');
        });
    });
    ...
    function exportElementAsPDF(element, filename, ignoreSelector) {
        ...
        html2canvas(element, {
            ...
            let y = (pdfHeight - imgHeight) / 2; // 計算垂直置中的 Y 座標
