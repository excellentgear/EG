...
(function () {
  ...
  function build() {
    ...
    style.textContent = [
      ...
      // 已超預測回廠(P80)未回：只用「黑色粗框 + 內側白細框」標示邊緣，不蓋住文字；任何底色/轉圖片(PNG)都清楚
      '.cg-bar.due-over{border:2px solid #000;box-shadow:inset 0 0 0 1px #fff, 0 0 3px 1px rgba(0,0,0,.55);z-index:5;}',
  ...
  function render() {
    ...
    leg.innerHTML =
      ...
      '<span><i style="background:' + PRIO.n.bg + ';border:2px solid #000;box-shadow:inset 0 0 0 1px #fff"></i>⚠已超預測回廠(P80)未回</span>' +
  ...
  function exportImage() {
    ...
    L.groups.forEach(function (g) {
      ...
      g.segs.forEach(function (x) {
        ...
        ctx.globalAlpha = 1;
        if (isOverdue(r)) {
          ctx.lineWidth = 2; ctx.strokeStyle = '#000'; roundRect(ctx, bx, by, bw, BAR_H, 3); ctx.stroke();
          ctx.lineWidth = 1; ctx.strokeStyle = '#fff'; roundRect(ctx, bx + 1.5, by + 1.5, bw - 3, BAR_H - 3, 2); ctx.stroke();
        }
