...
<div class="ss-mask" id="helpUseMask"><div class="ss-modal" style="width:860px;">
    ...
    <div class="m-body help-doc">
        ...
        <ul>
            ...
            <li><b>「型式」是什麼</b>：同一個料號（或機台型號／量具）＋製程＋機台＋客戶底下再分的版本，
                例如<b>有隆齒／無隆齒</b>，或同一台機器的<b>上下料／架機／偏擺確認</b>。
                <b>幾種以內由管理員設定</b>（「設定 → 頻率／型式／注意事項 → 型式上限」，預設 3 種；
                留空＝「未分型式」，那本身也算一種）；建議選項在同一頁維護，仍然可以自己打。</li>
            <li><b>機台型號可以多選</b>（2026-09-23 新增）：一份 SOP 常常涵蓋好幾種型號的機器。
                挑到的型號底下的機台會全部帶進來再逐台勾掉不要的，清單上會標出每一台是哪個型號。
                <b>真正算數的是勾起來的機器編號</b>——型號多選了卻一台都沒勾的，存檔時會自動不算。</li>
            <li><b>綁量具時不會有「製程」「使用設備」「預計工時」</b>（2026-09-23 改）：
                量具的說明書講的是「這支量具怎麼用」，跟走到哪一關製程無關，
                上面的量具編號本身就是它用的設備。<b>型式一樣可以分</b>。</li>
        ...
        <ul>
            ...
            <li><b>精度等級只能從對照表挑</b>（2026-09-23 新增）：把預設值那一列的「輸入方式」設成
                <b>齒輪精度等級</b>，文件上那一格就變成唯讀＋「挑等級」——
                <b>① 先選標準（ISO／DIN／JIS／AGMA）② 再點等級數字</b>，填進去的是「AGMA 11」這種完整字串。
                等級清單<b>即時取自主檔管理 →「類別字典設定」→ 齒輪等級</b>的對照表，
                要增減等級請到那邊改，這一頁會跟著變（這裡不另外存一份）。</li>
