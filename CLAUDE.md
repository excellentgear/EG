...
> 最後修改：2026-08-03（三次）— `ai-rules/16` 新增**第一之三節「AS 文件編號綁定的做法全站統一」**（後端唯一實作 `src/common/asdoc_lib.php`、前端唯一實作 `resource/js/eg_asdoc_picker.js`＝打編號即時篩選＋清單；**禁止純下拉、禁止各頁自刻**；表頭/表尾唯讀反灰由綁定推導）。起因：AS 文件已 163 份，教育訓練、報價單等頁面的綁定 UI 只是一個長下拉，使用者要在 163 個選項裡用眼睛找編號；且每頁存放位置與顯示格式都不一樣。同時「品管/生產/技術部門」三處模組內設定全部改為**反灰唯讀**，只能在組織角色綁定設定頁改。
...
| 要做/改任何「列印文件」（報表、表單、清單列印版）或**任何簽章欄** | `ai-rules/16-列印文件標準.md`（**必讀**，大標題本公司名動態取禁寫死／**表頭＝已綁定 AS 文件的表單名稱 doc_name，禁寫死**／頁碼左下 counter(pages)／AS 文件編號右下角／**簽章一律走 `eg_stamp.js` 帶日期印章、代理人代簽右下角加「代」字＝第三之二節**） |
