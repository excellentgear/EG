...
> 最後修改：2026-08-04 — 鐵律新增**第8條「修改/建立一律用最省 token、最聰明快速完美的方式」**（使用者明確要求，全程適用；原散落於 UI 規則末段的「工作方式」併入此條，避免重複）；同條內附**附件標籤鐵則**（上傳附件沒勾類別標籤一律不准存檔，前端即時擋＋後端 API 同步擋雙保險）。起因：`master_data_management.php` 的料號附件上傳可以完全不選標籤就存檔，之後附件會混在一起難以分類查找；順便把使用者對「工作方式」的既有要求正式收斂進鐵律，不再只是散在 UI 規則裡的一句話。
