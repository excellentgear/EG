...
> 最後修改：2026-08-13 — 審核表單（review_form）三項使用者明確要求的改動：①負責單位/負責人挑選器可選到**兼任人員**——`people_lib.php` 的 `eg_people_list()` 新增 `dept_ids` 欄位（該人所有部門id含兼任，不只主要部門），全站其他 15 支呼叫端純加欄位不受影響。②新增「小項」功能：一個項目可拆多個小項；欄位/負責部門/負責人/簽名如何處理歷經兩輪討論才定案——**最終版是負責部門/負責人與簽名都改成掛在小項上各自獨立設定、各自簽**（不是掛在項目上整個共用），因此新增 `rf_instance_subitem` 子表（項目 `rf_instance_item` 降級為純項次分組容器），`rf_instance_item_confirm.item_id` 改名 `subitem_id`（改名當下 0 筆舊資料安全），既有 4 筆舊項目已遷移進新表未遺失；畫面上每個小項一整列都是完整獨立欄位，沒有 rowspan 合併，只有項次編號與「刪除整個項次」鈕只在該項目第一列顯示。③日期／下拉選單自訂欄位表頭新增「整欄套用」，選一次值一鍵套用到目前所有項目/小項的同一欄，取代逐列手填相同值。詳見記憶 `review_form_module`。
