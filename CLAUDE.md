...
| 待辦：把供應商稽核／教育訓練模組的 AS 編號版次串上「依業務日期回推」機制 | `ai-rules/work/待補-AS編號版次依業務日期回推.md`（**一次性任務交接檔，做完驗證過要依檔內指示自行刪除，並移除本路由表這一列**） |
