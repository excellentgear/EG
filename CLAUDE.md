...
> 最後修改：2026-08-06 — 新增 `ai-rules/19-核准人員解析規則.md`（三段式：綁部門＝該部門任一職級不低於送出者的主管皆可核准／綁人員＝固定該人／都未設＝自動抓表單綁定的AS文件所屬部門套用同一套規則）並登記進路由表；`org_role_lib.php` 新增 `dept_or_user` 綁定類型與對應設定頁區塊。使用者明確要求，起因供應商稽核計劃的核准人原本寫死用全站共用的單一 `top_approver`，但這張表單需要自己的核准規則，不該每個表單都共用同一個人。
