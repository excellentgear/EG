...
> 最後修改：2026-08-03 — UI 規則新增**長清單下拉一律可打字篩選**（`data-eg-filter`＋`eg_input_rules.js` 規則 7，禁止各頁自刻）；使用者明確指出這點**已經重複犯很多次**，所以從口頭要求升格成寫進本檔的鐵則。同時組織角色綁定改為**含子部門認列**（`eg_org_dept_ids()`／`eg_org_in_dept()`，路由表該列已更新）並新增「倉管部門」；修掉 `eg_org_save()` 內呼叫 `eg_org_ensure_schema()` 造成 DDL 隱式 commit、外層 `commit()` 爆 "There is no active transaction" 的儲存失敗 500。
