...
/* AS稽核查檢表＝**全自動、內容一律唯讀**（2026-09-22 使用者要求）。
   合格／不合格、所見證據或建議、備註全部由系統依「系統稽核紀錄表」自動帶入
   （ia_as_apply_system_result()），不提供人工填寫、不提供增刪項目，
   要開不符合通知單也一律回到系統稽核紀錄表那一張去開（那邊本來就有這個功能）。
   前端把欄位反灰，這裡用同一條規則再擋一次（鐵律8）；訊息只有一份，避免兩邊說法不一樣。 */
const IA_AS_READONLY_MSG = 'AS稽核查檢表的內容由系統自動判定帶入，不提供人工修改或增刪項目；'
    . '要更新判定請用「重新自動判定」重新選一次來源的系統稽核紀錄表。';
