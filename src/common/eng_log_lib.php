...
function el_channels(): array {
    return ['phone' => '電話', 'email' => 'email', 'onsite' => '現場', 'im' => '通訊軟體', 'other' => '其他'];
}
...
{
    try {
        $st = $db->prepare("SELECT 1 FROM eng_log_index WHERE log_id = ? AND part_d_id IS NOT NULL LIMIT 1");
        $st->execute([$logId]);
        return (bool)$st->fetchColumn();
    } catch (Throwable $e) { return false; }
}
