...
case 'plan_save':
    ...
    // 送簽之後「編排」鎖定，只有管理員能改（使用者 2026-09-24 要求：至少要讓管理員改得動已送簽的
    // 預計開始／預計完成日期）——前端把編輯器整組藏起來、只留管理員的「解鎖編排」，這裡是後端同規則
    // 再擋一次（鐵律8），否則任何有編輯權的人直打這支 API 一樣能改到已送簽甚至已核准的排程。
    if (prj_plan_locked($prjPlan) && !$P['canAdmin']) {
        jerr('這份規劃表已經送簽，編排已鎖定；只有管理員可以修改（請在「執行規劃表」按「解鎖編排」）', 403);
    }
