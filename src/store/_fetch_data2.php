...
// c:\MAMP\htdocs\EGsystem\src/store/_fetch_data2.php
// --- 強制清除所有舊緩衝並關閉壓縮 ---
if (ob_get_level()) {
    ob_end_clean();
}
...
if (is_array($OreadyReply_list_base)) {
    ...
    if (!empty($all_d_ids_for_orders)) {
        ...
        try {
            ...
            error_log("Error fetching order_list in _fetch_data2.php: " . $e->getMessage());
        }
    }
    ...
    try {
        ...
        // 主頁面初始載入有這段邏輯，但 _fetch_data2.php 原本沒有，導致自動刷新後報工資訊消失
        $pm_report_map   = [];
        $pm_schedule_map = [];
        $finished_fids_set = [];
        $bom_all_fids_map  = [];
        ...
        error_log("Error fetching sub-details in _fetch_data2.php: " . $e->getMessage());
