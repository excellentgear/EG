...
// ====== PATCH結束 ======
    ...
    $response['total_qq_qty'] = ($qq_sum_result && $qq_sum_result['total_qq_qty'] !== null) ? (float)$qq_sum_result['total_qq_qty'] : 0;
        ...
        $response['qc_ps'] = $final_bom_ing_state['QC_ps'] ?? '';
        $response['qc_ps2'] = $final_bom_ing_state['QC_ps2'] ?? '';
        $response['qc_ps_aod'] = $final_bom_ing_state['QC_ps_aod'] ?? '';
        $response['main_total_qty'] = isset($final_bom_ing_state['sqty']) ? (float)$final_bom_ing_state['sqty'] : 0;
    } else {
