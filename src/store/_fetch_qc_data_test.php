<?php
// src/store/_fetch_qc_data_test.php
...
WHERE
    ...
    --   （或直接以 _fetch_qc_data_test.php.bak_20260623_before_currentprocess 覆蓋回去）。
    -- ★ 2026-07-14 改成比對 bom_sn 而非日期本身：拆批後同一 bom_sn 底下的各批次
    --   outsource_date 可能不同（分次發給不同廠商），舊寫法只認「日期剛好等於全 bom 最大值」
    --   那一批，導致同一 bom_sn 其他已回廠(Q)的批次被排除、QC待驗清單看不到。
    --   改為先找出「目前製程」是哪一個 bom_sn，該 bom_sn 底下所有符合狀態的批次都列入。
    AND COALESCE(bi.bom_sn, -1) = (
        SELECT COALESCE(cur.bom_sn, -1)
        FROM bom_ing cur
        WHERE cur.bom = bi.bom
          AND cur.processing_state IN ('Q','P','ing','E')
          AND cur.outsource_date IS NOT NULL
          AND cur.is_schedule_split = 0
          AND cur.is_consumed = 0
        ORDER BY cur.outsource_date DESC, COALESCE(cur.bom_sn, -1) DESC
        LIMIT 1
    )
    $extraWhere
