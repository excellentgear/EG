...
if (isset($_POST['BOMSetting_checkBom'])){
    ...
    header("location:../../views/pm/New_Bom.php?bt=bcb");
};
...
if (isset($_POST['BOMSetting_UpDate'])){
    ...
    IF($M==null){
        header("Location:../../views/pm/New_Bom.php?message=BOM 格式錯誤 或 資料有誤");
    } else {
        ...
        if ($bom == $_SESSION['BOM']) {
            if($_GET['bt']='bcb'){
                ...
                header("Location:../../views/pm/New_Bom.php?message=success");
            }else{
                header("Location:../../views/pm/New_Bom.php?message=BOM 重複設定");
            }
            
        } else {
            $sth = $db->prepare("INSERT INTO `BOM`(sqty,`bom`,`d_id`,`Client_Name`,`Created_By`,Created_At)
            VALUES('$_POST[sqty]','$bom','$_POST[d_id]','$_POST[Client_Name]','$_SESSION[id]','$today')");
            $sth->execute();
...
if (isset($_POST["resetpSetting"])) {
    ...
    header("Location:_cleanNewBom.php");
};

// 刪除
if (isset($_POST["bom_Del"])) {
    $_SESSION['bom_Del']=9;
    header("location:../../views/pm/New_Bom.php?&message=請確認是否刪除 ".$_SESSION['bom']);
};
...
if (isset($_POST["bom_Del_dbCHECK"])) {
    ...
    header("location:../../views/pm/New_Bom.php?bom=".$_POST['BOM']);
