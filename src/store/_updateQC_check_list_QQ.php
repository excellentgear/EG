...
// New: Handle fetch request for QQ details
if (isset($_GET['action']) && $_GET['action'] === 'fetch_qq_details' && isset($_GET['bi'])) {
    ...
    try {
        // ⭐ 修正：確保 $db 物件在此 AJAX 請求的上下文中可用
        if (!isset($db) || !($db instanceof PDO)) {
            if (class_exists('DBConnection')) {
                $connInstance = new DBConnection();
                $db = $connInstance->getDB();
            }
            if (!isset($db) || !($db instanceof PDO)) {
        ...
        $sql_fetch_qq = "SELECT qc_check_id, QC_QQ_sqty, QC_ps,
                                DATE_FORMAT(QC_check_date, '%m/%d') AS QC_check_date_formatted 
                         FROM qc_check
                         WHERE bom_ing_fid_ref = :bom_ing_fid AND QC_check = 'QQ' 
                         ORDER BY qc_check_id ASC";
        $stmt_fetch_qq = $db->prepare($sql_fetch_qq);
        $stmt_fetch_qq->bindParam(':bom_ing_fid', $bom_ing_fid_to_fetch, PDO::PARAM_STR);
        $stmt_fetch_qq->execute();
        $qq_records = $stmt_fetch_qq->fetchAll(PDO::FETCH_ASSOC); // Changed variable name

        $response['success'] = true;
        $response['data'] = $qq_records; // Use the correct variable
        $response['message'] = 'QQ details fetched successfully.'; // Optional: more specific message for fetch
        echo json_encode($response);
        exit;

    } catch (Exception $e) { // Catch PDOException and general Exception
        $response['message'] = 'Error fetching QQ details: ' . $e->getMessage();
...
$qq_quantities_post = $_POST['qq_total_qty'] ?? [];
$qc_messages_post = $_POST['QCmessage'] ?? [];
$qc_check_ids_post = $_POST['qc_check_id'] ?? [];

if (empty($bom_ing_fid)) {
    error_log("[QC_QQ] Error: Received empty bom_ing_fid. User_id: '" . $user_id . "'");
    $response['message'] = '錯誤：bom_ing_fid 為空或缺失。'; // Translated message
    echo json_encode($response);
    exit;
}

error_log("[QC_QQ] Processing bom_ing_fid: '" . $bom_ing_fid . "', user_id: '" . $user_id . "'");

try {
    $db->beginTransaction();

    if ($clear_remark_only) {
        // Action: Clear existing QQ records from qc_check
        $sql_delete_qc_checks = "DELETE FROM qc_check WHERE bom_ing_fid_ref = :bom_ing_fid AND QC_check = 'QQ'";
        ...
        if (!$cmd_delete_qc_checks->execute()) {
            $response['message'] = '清除異常檢驗記錄失敗: ' . $cmd_delete_qc_checks->errorInfo()[2];
            $db->rollBack();
            echo json_encode($response);
            exit;
        }
        ...
        $response['message'] = '異常紀錄已清除。';

} else {
        // Action: Add/Update QQ records in qc_check and update bom_ing
        if (!is_array($qq_quantities_post) || !is_array($qc_messages_post) || !is_array($qc_check_ids_post) ||
            count($qq_quantities_post) !== count($qc_messages_post) ||
            count($qq_quantities_post) !== count($qc_check_ids_post)) {
            $db->rollBack();
            $response['message'] = '提交的異常數據格式不正確。';
            echo json_encode($response);
            exit;
        }
...
for ($i = 0; $i < count($qq_quantities_post); $i++) {
            $qty_str = isset($qq_quantities_post[$i]) ? trim($qq_quantities_post[$i]) : '';
 ...
 $qty_num = filter_var($qty_str, FILTER_VALIDATE_FLOAT);
         ...
         if (is_numeric($current_qc_check_id) && (int)$current_qc_check_id > 0) {
                ...
                $sql_fetch_existing = "SELECT QC_QQ_sqty, QC_ps FROM qc_check WHERE qc_check_id = :qc_check_id AND bom_ing_fid_ref = :bom_ing_fid_ref";
                ...
                if ($existing_record) {
                    $db_qty = isset($existing_record['QC_QQ_sqty']) ? (float)$existing_record['QC_QQ_sqty'] : null;
                    $db_ps_normalized = ($existing_record['QC_ps'] === null) ? '' : $existing_record['QC_ps'];
                    ...
                    $ps_is_different = ($db_ps_normalized !== $msg_str_normalized);

                    if ($qty_is_different || $ps_is_different) {
                        $perform_db_update = true;
                    }
                } else {
                    // qc_check_id provided, but no record found. This could be an anomaly or a new row with a pre-filled (but not yet saved) ID.
                    // If it's truly a new row scenario, the INSERT path should handle it.
                    // If it's an ID for a non-existent/deleted record, an UPDATE would fail anyway.
                    // For safety, we can log this. If the form intends this as an update, we might proceed.
                    error_log("[QC_QQ_UPDATE_WARN] User: $user_id, QC_ID: $current_qc_check_id not found for bom_ing_fid: $bom_ing_fid. This ID might be for a new or deleted record.");
                    // $perform_db_update = true; // Or decide to skip if record must exist
                }
                ...
                if ($perform_db_update) {
                    if ($isPrivilegedUser) {
                        ...
                        $sql_update_qc_check = "UPDATE qc_check SET QC_ps = :qc_ps, QC_QQ_sqty = :qc_QQ_sqty, QC_check_date = NOW(), updated_by = :user_id, updated_at = NOW() WHERE qc_check_id = :qc_check_id AND bom_ing_fid_ref = :bom_ing_fid_param";
                        $cmd_qc_action = $db->prepare($sql_update_qc_check);
                        $cmd_qc_action->bindParam(':qc_ps', $msg_str, ($msg_str === '') ? PDO::PARAM_NULL : PDO::PARAM_STR);
                        $cmd_qc_action->bindParam(':qc_QQ_sqty', $qty_num, PDO::PARAM_STR); // qty_num is float or null
                    } else {
                    ...
                    if (!$cmd_qc_action->execute()) {
                        $db->rollBack();
                        $response['message'] = '更新異常記錄 (ID: ' . $current_qc_check_id . ') 失敗: ' . $cmd_qc_action->errorInfo()[2];
                        error_log("[QC_QQ_UPDATE_FAIL] User: $user_id, QC_ID: $current_qc_check_id, Error: " . $cmd_qc_action->errorInfo()[2]);
                        echo json_encode($response);
                        exit;
                    }
                } else {
                ...
                $sql_insert_qc_check = "INSERT INTO qc_check (
                                        bom_ing_fid_ref, QC_check, QC_check_date, QC_ps, QC_QQ_sqty,
                                        created_by, created_at, updated_by, updated_at
                                    ) VALUES (
                                        :bom_ing_fid_param, 'QQ', NOW(), :qc_ps, :qc_QQ_sqty,
                                        :user_id, NOW(), :user_id, NOW()
                                    )";
                $cmd_qc_action = $db->prepare($sql_insert_qc_check);
                $cmd_qc_action->bindParam(':bom_ing_fid_param', $bom_ing_fid, PDO::PARAM_STR);
                $cmd_qc_action->bindParam(':qc_ps', $msg_str, ($msg_str === '') ? PDO::PARAM_NULL : PDO::PARAM_STR);
                $cmd_qc_action->bindParam(':qc_QQ_sqty', $qty_num, PDO::PARAM_STR);
                $cmd_qc_action->bindParam(':user_id', $user_id, PDO::PARAM_INT);

                if (!$cmd_qc_action->execute()) {
                    $db->rollBack();
                    $response['message'] = '新增異常記錄至 qc_check 失敗: ' . $cmd_qc_action->errorInfo()[2];
                    error_log("[QC_QQ_INSERT_FAIL] User: $user_id, BOM_ING_FID: $bom_ing_fid, Error: " . $cmd_qc_action->errorInfo()[2]);
                    echo json_encode($response);
                    exit;
                }
...
// Delete orphaned qc_check 'ok' records
        if (!empty($processed_qc_check_ids)) {
            ...
            $sql_delete_orphans = "DELETE FROM qc_check 
                                   WHERE bom_ing_fid_ref = ? AND QC_check = 'QQ' AND qc_check_id NOT IN ($placeholders)";
            ...
            $sql_delete_orphans = "DELETE FROM qc_check 
                                   WHERE bom_ing_fid_ref = ? AND QC_check = 'QQ'";
            $cmd_delete_orphans = $db->prepare($sql_delete_orphans);
            $delete_params = [$bom_ing_fid];
        }
        if (!$cmd_delete_orphans->execute($delete_params)) {
            $db->rollBack();
            $response['message'] = '刪除舊異常記錄失敗: ' . $cmd_delete_orphans->errorInfo()[2];
            echo json_encode($response);
            exit;
        }

        $response['message'] = '異常記錄更新成功。';
    }
    ...
    $stmt_fetch_bom_ing = $db->prepare("SELECT QC_check, DATE_FORMAT(QC_check_date, '%m/%d') as QC_check_date_formatted, processing_state, QC_ps, QC_ps2, QC_ps_aod, sqty FROM bom_ing WHERE bom_ing_fid = :bom_ing_fid_fetch");
