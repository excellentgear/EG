...
// 單一 BOM 的逐關製程進度：唯一實作已抽到 src/common/bom_progress_lib.php（鐵律4），
// 工程處理紀錄(eng_log)也要顯示同一份製程條，兩邊共用同一套口徑。這裡保留原函式名當薄包裝，
// 本檔其餘呼叫端一行都不必改。
require_once __DIR__ . '/../common/bom_progress_lib.php';
function pt_bom_progress(PDO $db, string $bom): ?array { return eg_bom_progress($db, $bom); }
